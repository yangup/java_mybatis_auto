	...
	private void whereCreate() {
		try {
			for (int index = 0; index < list.size(); index++) {
				...
				writer.write(CharUtil.getTab(4) + "AND " + cloumn.toLowerCase()
						+ CharUtil.getWhitespace(MaxLength - cloumn.length()));
				writer.write(" = ");
				writer.write("#{" + field + "}" + "\n");
	...
	private void updateCreate() {
		try {
			...
			for (int index = 0; index < list.size(); index++) {
				...
				writer.write(
						CharUtil.getTab(3) + temp.toLowerCase() + CharUtil.getWhitespace(MaxLength - temp.length()));
				writer.write(" = ");
				writer.write("#{" + field + "}" + "\n");
			}
			isFirst = true;
			if (pris != null) {
				...
				writer.write(CharUtil.getTab(3) + temp + cloumn.toLowerCase()
						+ CharUtil.getWhitespace(MaxLength - cloumn.length()) + "= #{" + field + "}\n");
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private void updateByParamCreate() {
		try {
			...
			for (int index = 0; index < list.size(); index++) {
				...
				String field = CharUtil.UnderlineToUppercase(cloumn);
				writer.write(CharUtil.getTab(3) + "<if test=\"@Ognl@isNotEmpty(" + field + ")\">\n");
				writer.write(CharUtil.getTab(4) + cloumn.toLowerCase()
						+ CharUtil.getWhitespace(MaxLength - cloumn.length()));
				writer.write(" = ");
				writer.write("#{" + field + "}" + ",\n");
				writer.write(CharUtil.getTab(3) + "</if>\n");
